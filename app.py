...
div[data-testid='stFileUploader'] section button[data-testid='baseButton-secondary'] { font-size: 0 !important; background-color: #111111 !important; color: transparent !important; border: 1px solid #111111 !important; border-radius: 4px !important; padding: 0.35rem 1.2rem !important; transition: all 0.2s ease !important; }
div[data-testid='stFileUploader'] section button[data-testid='baseButton-secondary']::after { content: 'Examinar archivos' !important; font-family: 'Space Grotesk', sans-serif !important; font-size: 0.85rem !important; font-weight: 600 !important; color: #FFFFFF !important; display: inline-block !important; }
div[data-testid='stFileUploader'] section button[data-testid='baseButton-secondary']:hover { background-color: #FACC15 !important; border-color: #FACC15 !important; }
div[data-testid='stFileUploader'] section button[data-testid='baseButton-secondary']:hover::after { color: #111111 !important; }
...
with tab_app:
    ...
    if modo_entrada == "Cargar archivo CSV":
        ...
        if archivo_subido:
            try:
                ...
                st.success(f"Archivo cargado: **{len(df_devops)}** historias de usuario detectadas.")
            except Exception as e:
                st.error(f"Error al leer el archivo CSV: {e}")
    else:
        st.info("📦 **Modo demo activado:** historias de ejemplo con código HTML y comentarios de desarrollo.")
        datos_demo = {
            "ID": [1042, 1043, 1045],
            "Title": [
    ...
    if df_devops is not None:
        ...
        df_vista_limpia = df_devops[[col_id, col_title, "Description_Clean", "Acceptance_Criteria_Clean"]].copy()
        ...
        st.caption("Gemini extraerá los botones, campos y mensajes visibles y generará la propuesta de traducción.")
            
        if st.button("Identificar y traducir"):
            if not api_key_activa:
                st.error("Introduce tu clave Gemini API en la barra lateral para continuar.")
            else:
                with st.spinner("Analizando historias y generando propuesta con Gemini 3.6 Flash..."):
                    ...
                    try:
                        ...
                        if 'confianza' in df_res.columns:
                            df_res['confianza'] = df_res['confianza'].fillna(50)
                        ...
                        cols_existentes = [c for c in orden_cols if c in df_res.columns]
                        st.session_state["df_resultado"] = df_res[cols_existentes]
    ...
    if "df_resultado" in st.session_state and st.session_state["df_resultado"] is not None:
        ...
        st.caption("💡 Puedes hacer doble clic en cualquier celda para corregir o ajustar las traducciones antes de descargar.")
        ...
        csv_data = df_editado.to_csv(index=False, sep=";").encode('utf-8-sig')
...
with tab_guia:
    ...
    card_faq_html = (
        ...
        "<div style='font-size: 0.88rem; line-height: 1.6; color: #333333;'>"
        "<p style='margin-bottom: 8px;'><b>¿Necesito una clave API para usar LimpiaText?</b><br>"
        "No para probar el <b>Modo demo</b>, que funciona al instante sin configuración previa. "
        "Si vas a subir un <b>archivo CSV propio</b>, solo necesitas introducir una clave gratuita de Gemini (obtenida en <a href='[https://aistudio.google.com/app/api-keys](https://aistudio.google.com/app/api-keys)' target='_blank' style='color: #0066CC; text-decoration: underline; font-weight: 500;'>Google AI Studio &#8599;</a>) en la barra lateral.</p>"
        "<p style='margin-bottom: 8px;'><b>¿Por qué no se extraen las explicaciones largas de las historias?</b><br>"
        "LimpiaText descarta la narrativa técnica y los comentarios de desarrollo para quedarse únicamente con los elementos que el usuario final verá en la pantalla.</p>"
